...
function PricingAndProfitSettingsPage({ getShowSetting, handleUpdate, handleSubmit }) {
  const { t } = useTranslation()
  const [open, setOpen] = useState(false)

  const allCategories = getShowSetting?.parcel_categories || []
  const selectedCategories = allCategories.filter((cat) => cat.selected)

  let displayCategory = t('All types')
  if (allCategories.length > 0) {
    if (selectedCategories.length === allCategories.length) {
      displayCategory = t('All types')
    } else if (selectedCategories.length === 0) {
      displayCategory = t('without limit')
    } else if (selectedCategories.length === 1) {
      displayCategory = selectedCategories[0].name
    } else {
      displayCategory = selectedCategories.map((c) => c.name).join(', ')
    }
  }
