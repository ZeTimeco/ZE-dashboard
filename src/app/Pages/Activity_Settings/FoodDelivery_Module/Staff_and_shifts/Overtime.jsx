...
function Overtime({setOpenDetails , getStaffManageConfig}) {
  ...
  return (
    <div className='shadow-[0_0_4px_0_rgba(0,0,0,0.20)] p-4'>
      ...
      {getStaffManageConfig?.out_shift?.map((outShift)=>(
        <motion.div
          key={outShift?.id}
          onClick={() => setOpenDetails(outShift?.id)}
          className='border border-[#E3E8EF] rounded-3px p-4 mt-4 flex justify-between cursor-pointer overflow-hidden relative'
          whileHover={{
            y: -2,
            boxShadow: '0 4px 12px rgba(249, 112, 102, 0.15)',
          }}
          whileTap={{ scale: 0.97 }}
        ...
        >
          ...
          <motion.span
            ...
            style={{ background: 'radial-gradient(circle, rgba(249,112,102,0.25) 0%, transparent 70%)' }}
          />
          ...
          <div className='flex gap-2'>
            ...
            <div>
              <p className='text-[#364152] text-base font-normal'>{outShift?.name}</p>
              <p className='text-[#4B5565] text-sm font-normal'> {outShift?.role}</p>
            </div>
          </div>

          {/*  */}
          <div className='flex items-center'>
            {outShift?.status === 'inactive' ? (
