...
function Shipment_details({getActiveDeliveryID}) {
  ...
  return (
    ...
    <div className='border border-[#CDD5DF] rounded-3px p-6  grid grid-cols-2 gap-6'>
      ...
      {getActiveDeliveryID?.dropoffs?.items?.map((item)=>(
        <div key={item?.id} className='border border-[#CDD5DF] rounded-3px p-4'>
          <div className='flex justify-between'>
            ...
            <p className='w-fit bg-[#F5DFA3] text-[#5C460A] text-sm font-normal px-2 py-1 rounded-full'>
              {t('Destination')} {getArabicOrdinal(item?.stop_order)}
            </p>
          </div>
          ...
          <div className='flex justify-between'>
            ...
            <p className='text-[#161616] text-sm font-normal'>  {item?.category}</p>
          </div>

          {/* the weight */}
          <div className='flex justify-between mt-4'>
            <p className='text-[#697586] text-sm font-normal'>{t('the weight')}</p>
            <p className='text-[#161616] text-sm font-normal'>  {item?.parcel_weight_kg} كجم </p>
          </div>
