...
import { DeleteItemThunk, getMenusThunk } from '@/redux/slice/Menus/MenusSlice';

function Delete({open , setOpen , itemId , setOpenDetails}) {
    const {t} = useTranslation();
    const dispatch = useDispatch()

    const handleDelete = async () => {
      if (itemId) {
        const result = await dispatch(DeleteItemThunk(itemId))
        if (!result.error) {
      ...
      >
        ...
        <div className='flex flex-col  items-center mb-6'>
          <p className='text-[#4B5565] text-lg font-medium mb-2'>
            {t('Are you sure about deleting it?')}
          </p>
        ...
        <section className='w-full flex p-6 gap-3'>
          ...
          >
            <span className='text-base font-medium'>{t('Confirmation of deleting')}</span>
          </button>
          <button
            onClick={()=>setOpen(false)}
