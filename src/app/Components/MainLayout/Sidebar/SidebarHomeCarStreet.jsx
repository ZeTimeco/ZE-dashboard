...
function SidebarHomeCarStreet({ isSidebarOpen, setIsSidebarOpen }) {
  ...
  const mainNavItems = [
    ...
    { key: 'Requests',          path: '/Pages/requests/Home_Car_Street_Module', icon: '/images/icons/Requests.svg' },
  ...
  const NavItem = ({ item }) => {
    ...
    return (
      <motion.li whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} transition={{ duration: 0.15 }}>
        <Link
          href={item.path}
      ...
      <div className="flex items-center justify-between px-4 mb-6 relative">
        <AnimatePresence mode="wait">
          {open ? (
            ...
            >
              ...
              <img src="/images/LogoText.svg" alt="Zetime" className="h-5" />
            </motion.div>
          ) : (
            <motion.div
              key="mini-logo"
        ...
        {open && (
          <motion.button
            ...
            whileTap={{ scale: 0.92 }}
            onClick={() => { if (window.innerWidth >= 1340) setOpen(false); }}
